                    ...
                    if (item.props.length) {
                        ...
                        item.props.forEach(function(prop) {
                            try {
                                ...
                                let tagType = typeof item.type === 'string' ? item.type : (item.type?.name || 'component');
                                let propValue = prop.expression || prop.value;
                                console.error('%c[LemonadeJS] Template Error', 'background: #ff0000; color: white; padding: 2px 6px; border-radius: 3px;');
                                console.error(`Tag: <${tagType}>`);
                                console.error(`Attribute: ${prop.name}="${propValue}"`);
                                console.error('Original error:', e.message);
                                throw new Error(`[LemonadeJS] <${tagType}> ${prop.name}="${propValue}" - ${e.message}`);
                            }
                        })
                    }
